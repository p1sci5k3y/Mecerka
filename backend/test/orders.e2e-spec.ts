...
describe('Order Lifecycle (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!PWD_VAL) {
      throw new Error(
        'Missing E2E_PASS environment variable for creating test users',
      );
    }
    ...
    const client = await prisma.user.findFirst({
      where: { email: `client_${suffix}@e2e.com` },
    });
    ...
    await request(app.getHttpServer())
      .post('/auth/register')
      .send({
        email: `prov_${suffix}@e2e.com`,
        password: PWD_VAL,
        name: 'Provider',
        role: Role.PROVIDER,
      });
    const prov = await prisma.user.findFirst({
      where: { email: `prov_${suffix}@e2e.com` },
    });
    await prisma.user.update({
      where: { id: prov!.id },
      data: { emailVerified: true },
    ...
    await request(app.getHttpServer())
      .post('/auth/register')
      .send({
        email: `run_${suffix}@e2e.com`,
        password: PWD_VAL,
        name: 'Runner',
        role: Role.RUNNER,
      });
    const runUsr = await prisma.user.findFirst({
      where: { email: `run_${suffix}@e2e.com` },
    });
    await prisma.user.update({
      where: { id: runUsr!.id },
      data: { emailVerified: true },
    ...
    const city = await prisma.city.create({
      data: { name: `City E2E ${suffix}`, slug: `city-e2e-${suffix}` },
    });
    const cat = await prisma.category.create({
      data: { name: `Cat E2E ${suffix}`, slug: `cat-e2e-${suffix}` },
    });
