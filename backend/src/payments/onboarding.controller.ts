import { Controller, Get, UseGuards, Request, Res, Query, BadRequestException, Logger } from '@nestjs/common';
...
export class OnboardingController {
    ...
    async getConnectLink(@Request() req: RequestWithUser) {
        const url = await this.paymentsService.generateOnboardingLink(req.user.userId);
        return { url };
    }
    ...
    async handleStripeCallback(
        ...
        if (!accountId) {
            throw new BadRequestException('Missing accountId from Stripe OAuth return');
        }
        ...
        try {
            ...
            return res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:3001'}/dashboard?stripe_connected=false&error=verification_failed`);
        }
    }
}
