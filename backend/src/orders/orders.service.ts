...
export class OrdersService {
  ...
  /**
   * @deprecated Use checkoutFromCart() instead. This method bypasses cart validation
   * and stock reservation. Will be removed in a future version.
   */
  async create(createOrderDto: CreateOrderDto, clientId: string) {
    this.logger.warn('Deprecated create() called', {
      clientId,
      source: 'manual-order',
    });
    ...
    const order = await this.prisma.order.create({
      data: {
        clientId,
        cityId,
        checkoutIdempotencyKey: `manual-${clientId}-${items
