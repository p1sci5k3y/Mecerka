...
describe('OrdersService - Saga Lite Payment Domain', () => {
    ...
    async function setupTestData(stockA: number, stockB: number) {
        ...
        const provA = await prisma.user.create({
            data: { email: `proA_${rand}@test.com`, password: 'pass', name: 'Prov A', roles: { set: [Role.PROVIDER] } },
        });

        const provB = await prisma.user.create({
            data: { email: `proB_${rand}@test.com`, password: 'pass', name: 'Prov B', roles: { set: [Role.PROVIDER] } },
        });

        const prodA = await prisma.product.create({
            data: { name: 'Prod A', price: 10, stock: stockA, providerId: provA.id, cityId: city.id, categoryId: cat.id },
        });

        const prodB = await prisma.product.create({
            data: { name: 'Prod B', price: 15, stock: stockB, providerId: provB.id, cityId: city.id, categoryId: cat.id },
        });
                    ...
                    create: [
                        {
                            ...
                            subtotal: qtyA * 10,
                            items: {
                                create: [{ productId: data.prodA.id, quantity: qtyA, priceAtPurchase: 10 }],
                            },
                        },
                        {
                            ...
                            subtotal: qtyB * 15,
                            items: {
                                create: [{ productId: data.prodB.id, quantity: qtyB, priceAtPurchase: 15 }],
                            },
                        },
                    ],
                },
    ...
    it('A) Confirmación decrementa stock y pasa a CONFIRMED', async () => {
        ...
        const result = await ordersService.confirmPayment(order.id, 'PAY_A');
        ...
        expect(dbOrder!.paymentRef).toBe('PAY_A');

        // Verify Stock physically decremented
        const pA = await prisma.product.findUnique({ where: { id: data.prodA.id } });
        const pB = await prisma.product.findUnique({ where: { id: data.prodB.id } });
    ...
    it('B) No hay stock → ProviderOrder rechazado + order sigue (Partial failure)', async () => {
        ...
        const result = await ordersService.confirmPayment(order.id, 'PAY_B');
        ...
        // DB Verification
        const poList = await prisma.providerOrder.findMany({ where: { orderId: order.id } });
    ...
    it('C) Todos rechazados → Order CANCELLED', async () => {
        ...
        const result = await ordersService.confirmPayment(order.id, 'PAY_C');
    ...
    it('D) Concurrencia: dos confirmPayment a la vez no generan overselling', async () => {
        ...
        const p1 = ordersService.confirmPayment(order1.id, 'PAY_CONCURRENT_1');
        const p2 = ordersService.confirmPayment(order2.id, 'PAY_CONCURRENT_2');
